package com.baidu.jinbao.innerapi.common.page;
...
public class SqlUtil {
    ...
    public abstract static class SimpleParser implements Parser {
    ...
    private class MyProviderSqlSource implements SqlSource {
        ...
        public BoundSql getBoundSql(Object parameterObject) {
            ...
            if (count) {
                ...
                return new BoundSql(configuration, sqlParser.getPageSql(boundSql.getSql()), getPageParameterMapping(
    ...
    private MappedStatement getMappedStatement(MappedStatement ms, SqlSource sqlSource, Object parameterObject,
        ...
        try {
            ...
            logger.info("No mappedStatement.");
        }
        if (qs == null) {
    ...
    private SqlSource getsqlSource(MappedStatement ms, SqlSource sqlSource, Object parameterObject, String suffix) {
        ...
        if (isDynamic(ms)) {
            ...
            if (sqlNode instanceof MixedSqlNode) {
                ...
                mixedSqlNode = new org.apache.ibatis.builder.xml.dynamic.MixedSqlNode(contents);
            }
            return new MyDynamicSqlSource(ms.getConfiguration(), mixedSqlNode, suffix.equals(SUFFIX_COUNT));
        } else if (sqlSource instanceof ProviderSqlSource) {
            return new MyProviderSqlSource(ms.getConfiguration(), (ProviderSqlSource) sqlSource,
                    suffix.equals(SUFFIX_COUNT));
        }
        // 如果是静态分页sql
        else if (suffix.equals(SUFFIX_PAGE)) {
            // 改为分页sql
    ...
    private SqlSource getPageSqlSource(Configuration configuration, SqlSource sqlSource, Object parameterObject) {
        BoundSql boundSql = sqlSource.getBoundSql(parameterObject);
        return new StaticSqlSource(configuration, sqlParser.getPageSql(boundSql.getSql()), getPageParameterMapping(
